...
export default function Insights() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      setReportSource(res.data.source || "ai");
      const now = new Date();
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (cached) {
          try {
            const { content, generatedAt, source } = JSON.parse(cached);
            setReport(content);
            setReportSource(source || null);
            ...
            <>
              {reportSource === "local" && (
                <p className="text-xs text-amber-600 dark:text-amber-400 mb-2">
                  Smart summary — Gemini quota unavailable. Add a valid API key for full AI reports.
                </p>
              )}
